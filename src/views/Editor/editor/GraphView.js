export default class GraphView {
  ...
  _listenMouseEvents() {
    ...
    this.canvas.addEventListener("mousedown", (e) => {
      this.observer["mousedown"] &&
        this.observer["mousedown"].forEach((callback) => {
          callback(e, this.getCanvasPointOfEvent(e));
        });
    });
    this.canvas.addEventListener("mouseup", (e) => {
      this.observer["mouseup"] &&
        this.observer["mouseup"].forEach((callback) => {
          callback(e);
        });
    });
    this.canvas.addEventListener("mousemove", (e) => {
      this.observer["mousemove"] &&
        this.observer["mousemove"].forEach((callback) => {
          callback(e);
        });
    });
    //画布失焦
    document.addEventListener("click", (e) => {
      if (e.target !== this.canvas) {
        this.observer["blur"] &&
          this.observer["blur"].forEach((callback) => {
            callback(e);
          });
        this.blur = true;
      } else if (this.blur) {
        this.blur = false;
        this.observer["focus"] &&
          this.observer["focus"].forEach((callback) => {
            callback(e);
          });
      }
    });
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        this.observer["delete"] &&
          this.observer["delete"].forEach((callback) => {
            callback(e);
          });
      }
      this.observer["keydown"] &&
        this.observer["keydown"].forEach((callback) => {
          callback(e);
        });
    });
  }
  ...
  removeEventListener(type, callback) {
    ...
    this.observer[type] = this.observer[type].filter(
      (item) => item !== callback
    );
  }
  draw() {
